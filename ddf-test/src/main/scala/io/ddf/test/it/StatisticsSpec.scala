...
trait StatisticsSpec extends BaseSpec with Matchers {
  ...
  feature("Statistics") {
    ...
    scenario("calculate vector mean") {
      val ddf = loadAirlineDDF()
      ddf.getVectorMean("Year") should not be null
    }


    scenario("calculate vector cor") {
      val ddf = loadAirlineDDF()
    ...
    scenario("calculate vector covariance") {
      val ddf = loadAirlineDDF()
    ...
    scenario("calculate vector variance") {
      val ddf = loadAirlineDDF()
      val variance = ddf.getVectorVariance("Year")
      variance.length should be(2)
    }

    scenario("calculate vector quantiles") {
      val ddf = loadAirlineDDF()
    ...
    scenario("calculate vector quantiles for double column") {
    ...
    scenario("calculate vector histogram") {
      val ddf = loadAirlineDDF()
