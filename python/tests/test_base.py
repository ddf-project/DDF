...
class BaseTest(unittest.TestCase):
    ...
    def loadAirlines(cls, dm):
        ...
        if table_name not in [x.split('\t')[0] for x in dm.sql('show tables')]:
            ...
            dm.sql('drop table if exists {}'.format(table_name))
            ...
            dm.sql("load data local inpath '{}/resources/test/airlineWithNA.csv' "
                   "into table {}".format(DDF_HOME, table_name))

        return dm.sql2ddf('select * from {}'.format(table_name))

    @classmethod
    def loadMtCars(cls, dm):
        table_name = 'mtcars_pyddf_unittest'
        if table_name not in [x.split('\t')[0] for x in dm.sql('show tables')]:
            dm.sql('set shark.test.data.path=resources', data_source='spark')
            # session.sql('set hive.metastore.warehouse.dir=/tmp')
            dm.sql('drop table if exists {}'.format(table_name), data_source='spark')
            dm.sql("CREATE TABLE {} (mpg double, cyl int, disp double, "
                   ...
                   " ROW FORMAT DELIMITED FIELDS TERMINATED BY ' '".format(table_name), data_source='spark')
            dm.sql("LOAD DATA LOCAL INPATH '{}/resources/test/mtcars' "
                        "INTO TABLE {}".format(DDF_HOME, table_name), data_source='spark')
        return dm.sql2ddf('select * from {}'.format(table_name), data_source='spark')
