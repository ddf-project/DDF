...
echo "#####################################"
pgrep -fl io.spark.ddf.examples >/dev/null 2>&1
if [ "$?" == "0" ]; then
	echo -e '\t Stopping DDF server ...'
	pkill -9 -f io.spark.ddf.examples >/dev/null 2>&1 || echo "Something wrong! DDF server is running but could not be killed."
fi
