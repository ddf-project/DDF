...
class RepresentationHandler(mDDF: DDF) extends RH(mDDF) {
  ...
  this.addConvertFunction(SCHEMARDD, RDD_ARR_OBJECT, new SchemaRDD2ArrayObject(this.mDDF))
  this.addConvertFunction(SCHEMARDD, RDD_ARR_DOUBLE, new SchemaRDD2ArrayDouble(this.mDDF))
  ...
  this.addConvertFunction(SCHEMARDD, RDD_REXP, new SchemaRDD2REXP(this.mDDF))
