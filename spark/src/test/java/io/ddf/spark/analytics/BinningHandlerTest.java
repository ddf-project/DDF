...
public class BinningHandlerTest extends BaseTest {
  ...
  public void testBinningToInteger() throws DDFException {
    ...
    AggregateTypes.AggregationResult ret = ddf1.xtabs(String.format("dayofweek, COUNT(*)"));
    ...
    ret = ddf2.xtabs(String.format("arrdelay, COUNT(*)"));
    Assert.assertEquals(ret.keySet().size(), 2);
    Assert.assertTrue(ret.keySet().containsAll(Arrays.asList(new String[] {"0", "1"})));
    ...
    ret = ddf3.xtabs(String.format("month, COUNT(*)"));
    Assert.assertEquals(ret.keySet().size(), 6);
    Assert.assertTrue(ret.keySet().containsAll(Arrays.asList(new String[] {"0", "1", "2", "3", "4", "5"})));
    ...
    Assert.assertTrue(ret.keySet().containsAll(Arrays.asList(new String[] {"0", "1", "2", "null"})));
  ...
  public void testBinningToLabels() throws DDFException {
    ...
    Assert.assertTrue(ddf3.getColumn("month").getOptionalFactor().getLevels().get().containsAll(Arrays.asList(new String[] {"[1,2]", "(2,4]", "(4,6]", "(6,8]", "(10,12]"})));

    ddf3 = ddf.binning("month", "custom", 0, new double[] { 2, 4, 6, 8 }, true, true, true);
    // {'[2,4]'=1, '(4,6]'=2, '(6,8]'=3}
    Assert.assertEquals(Schema.ColumnType.INT, ddf2.getSchemaHandler().getColumn("month").getType());
    Assert.assertTrue(ddf3.getColumn("month").getOptionalFactor().getLevels().get().containsAll(Arrays.asList(new String[] {"[2,4]", "(4,6]", "(6,8]"})));
