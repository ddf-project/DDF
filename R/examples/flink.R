...
sql(dm, "CREATE TABLE mtcars (mpg double, cyl int, disp double, hp int, drat double, wt double, qesc double, vs int, am int, gear int, carb string)", data.source="flink")
sql(dm, "LOAD '/tmp/mtcars' delimited by ' ' INTO mtcars", data.source="flink")
...
sql(dm, "CREATE TABLE flight (Year int,Month int,DayofMonth int, DayOfWeek int,
    ...
    WeatherDelay int, NASDelay int, SecurityDelay int, LateAircraftDelay int)", data.source="flink")

sql(dm, "LOAD '../../resources/test/airlineWithNA.csv' delimited by ',' into flight", data.source="flink")

sql(dm, "select count(*) from mtcars", data.source="flink")
ddf <- sql2ddf(dm, "select * from mtcars", data.source="flink")
...
sql(dm, "select count(*) from flight", data.source="flink")
jddf <- sql2ddf(dm, "select * from flight", data.source="flink")
